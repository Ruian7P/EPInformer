...
class promoter_enhancer_dataset(Dataset):
    def __init__(self, cell_type='K562', expr_type='RNA', n_enh_feats=3, disable_enh=False, distance_thr=None, max_n_enh=200, use_prm_signal=False, rm_prm_seq=False, motif_feat_path=None):
        ...
        if cell_type == 'K562':
            promoter_df = pd.read_csv('./data/K562_ABC_EGLinks/DNase_ENCFF257HEE_Neighborhoods/GeneList.txt', sep='\t', index_col='name')
    ...
    for use_rna_feats, rm_prm_seq in [(True, args.rm_prm_seq)]:
        for cell in [cell_type]:
            ...
            for n_enh_feats in [args.n_enh_feats]:
                ...
                ds = promoter_enhancer_dataset(cell_type=cell, expr_type=expr_type, n_enh_feats=n_enh_feats, distance_thr=dist_thr, max_n_enh=max_n_enh, use_prm_signal=use_prm_signal, rm_prm_seq=rm_prm_seq, motif_feat_path=motif_feat_path)
                ...
                if use_pretrained_encoder:
                    ...
                    model_kwargs = dict(n_extraFeat=n_enh_feats, pre_trained_encoder=pretrained_convNet.encoder, useFeat=use_rna_feats, out_dim=64, n_enhancer=max_n_enh, useBN=False, usePromoterSignal=use_prm_signal, motif_feat_dim=ds.motif_feat_dim)
                    model = model_dist[model_type](**model_kwargs).to(device)
                    ...
                    model_kwargs = dict(n_extraFeat=n_enh_feats, pre_trained_encoder=None, useFeat=use_rna_feats, out_dim=64, n_enhancer=max_n_enh, useBN=False, usePromoterSignal=use_prm_signal, motif_feat_dim=ds.motif_feat_dim)
                    model = model_dist[model_type](**model_kwargs).to(device)
